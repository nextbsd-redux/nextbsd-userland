...
/*
 * Gated open for the #91 process_message trace.
 *
 * These trace points fire per MESSAGE, and each one was doing
 * fopen/fprintf/fclose on syslogd's hot path. Acceptable for a 60-second CI
 * boot; on a real system it is a syscall storm and an unbounded file in /tmp.
 *
 * Off unless SYSLOGD_TRACE_PROCESS_MSG is set. Every caller already checks
 * the returned FILE * for NULL, so gating here disables all of them.
 */
static FILE *
_pm_trace_open(void)
{
	...
	if (enabled < 0)
		enabled = (getenv("SYSLOGD_TRACE_PROCESS_MSG") != NULL);
	if (enabled == 0)
		return NULL;
	return _pm_trace_open();
}
